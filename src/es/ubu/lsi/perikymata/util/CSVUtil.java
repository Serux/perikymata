...
public class CSVUtil {
		 ...
		 * </pre>
		 * 
		 * @param mainApp Reference to the main application, used to retrieve data.
		 * @param measure Measure object that gives the real size of the image.
		 * @param peaksCoords List of x,y coordinates.
		 * @param xDecileStart x coordinate of the first decile.
		 * @param xDecileEnd x coordinate of the last decile.
		 * @param decilesBetween x coordinates of the rest of deciles
		 * @throws IOException Thrown if there is an error writing the CSV file.
		 */
		public static void createCSV(MainApp mainApp, Measure measure, List<int[]> peaksCoords, double xDecileStart, double xDecileEnd, double[] decilesBetween ) throws IOException{
		   ...
		   //Vector distance from A to B of the measure
		    ...
		    //Decile size
		    int decilePixelSize = (int) (Math.max(decilesBetween[0],decilesBetween[1]) - Math.min(decilesBetween[0],decilesBetween[1]));    
	        ...
	        while(currentDecile <9){
	        	while(currentPerikymataIndex < peaksCoords.size() && decilesBetween[currentDecile] > peaksCoords.get(currentPerikymataIndex)[0]){
	        		writeDecileValues(writer, currentDecile, peaksCoords, currentPerikymataIndex, lastPerikymataIndex, measureTotalDist, measure);
	    		    numPerDecile++;
		 ...
		 * @param decile Current decile number.
		 * @param peaksCoords Coordinates of the perikymata.
		 * @param currentPerikymataIndex Current perikymata to write.
		 * @param lastPerikymataIndex Last perikymata, used to write the distance.
		 * @param measureTotalDist Vector distance between the two points of the measure.
		 * @param measure Measure object that gives the real size of the image.
		 * @throws IOException If error when writing on file.
		 */
		private static void writeDecileValues(FileWriter writer, int decile, List<int[]> peaksCoords,int currentPerikymataIndex,int lastPerikymataIndex, double measureTotalDist, Measure measure) throws IOException{
		 ...
		 * @param peaks List of the coordinates of the perikymata
		 */
